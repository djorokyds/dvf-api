...
function generateHTML(data, userLat, userLon) {
  ...
  const markersJS = transactions.map((t, i) => `
    L.circleMarker([${t.latitude}, ${t.longitude}], {
      ...
      <div style="font-family:sans-serif;min-width:160px">
        <strong>${i+1}. ${t.type_bien}</strong><br>
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .legend { display: flex; gap: 12px; padding: 0 14px 10px; font-size: 11px; color: #666; }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (format === 'html') {
      const html = generateHTML(responseData, lat, lon);
