...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .score-desc { font-size: 16px; color: #888; line-height: 1.6; max-width: 260px; margin: 0 auto; }
  </style>
</head>
<body>
  <div class="container">
    <div class="gauge-wrap">
      <svg viewBox="0 0 260 160">
        ...
        <g class="needle" id="needle">
          ...
          <circle cx="130" cy="150" r="10" fill="#2a2a2a" stroke="#555" stroke-width="1.5"/>
...
module.exports = async function handler(req, res) {
  const { adresse, type_bien, surface, prix_bien, tri, coc, payback, format } = req.query;

  const SUPABASE_URL = process.env.SUPABASE_URL;
  const SUPABASE_KEY = process.env.SUPABASE_KEY;

  if (!adresse) return res.status(400).json({ error: "adresse manquante" });
  if (!tri || !coc || !payback) return res.status(400).json({ error: "Paramètres scoring manquants (tri, coc, payback)" });
  ...
  try {
    ...
    const scoring = calculScoreOpportunite(triNum, cocNum, paybackNum, ecartPrix, nbTransactionsSection);

    const params = {
      adresse_normalisee, section_cadastrale, prix_median_m2,
      tri: triNum, coc: cocNum, payback: paybackNum,
