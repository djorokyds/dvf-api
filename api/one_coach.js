...
function renderLoadingPage(redirectUrl) {
  return `<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
<title>ONE Coach - Chargement</title>
...
module.exports = async function handler(req, res) {
  try {
    if (req.query.loading === '1') {
      const query = new URLSearchParams(req.query);
      query.delete('loading');

      const redirectUrl =
        '/api/one_coach' + (query.toString() ? `?${query.toString()}` : '');

      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      return res.status(200).send(renderLoadingPage(redirectUrl));
    }
